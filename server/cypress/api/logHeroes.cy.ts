  ...
  describe('requires auth', () => {
    ...
    it('can add hero', () => {
      ...
      cy.get('@response')
        .its('body')
        .then((newHero) => {
          console.log('Added hero:', newHero);
    ...
    it('can delete hero', () => {
      ...
      cy.get<HeroViewModel>('@newHero').then((newHero) => {
        ...
        console.log('Deleted hero:', newHero.id);
      });
    });
    ...
    it('should update hero', () => {
      ...
      cy.get<HeroViewModel>('@newHero').then((newHero) => {
        ...
        console.log('Updated hero:', newHero);
        cy.deleteHero(newHero.id);
      });
    });
    ...
    it('can upload avatar image', () => {
      ...
      cy.get<HeroViewModel>('@newHero').then((newHero) => {
        cy.fixture('avatar.png', 'binary').then((binaryString) => {
          ...
          cy.get('@response').its('status').should('equal', 200);
          console.log('Uploaded avatar image for hero:', newHero.id);
          cy.deleteHero(newHero.id);
        });
      });
    });
