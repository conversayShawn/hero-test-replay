...
describe('LoginForm', () => {
  ...
  it('FAILURE: when email and password are empty, validation messages should appear, but will not', () => {
    cy.mount(<LoginForm onLogin={cy.spy().as('onLoginSpy')} />);
    cy.get('button').click();
    cy.contains('Email is required');
    cy.contains('Password is required');
    cy.get('@onLoginSpy').should('have.been.called'); // should('not.have.been.called') will pass
  });
